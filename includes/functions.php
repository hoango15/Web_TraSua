...
function incrementFailedLoginAttempts($conn, $email)
{
    $sql = "SELECT * FROM login_attempts WHERE email = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $email);
    ...
    if ($result->num_rows > 0) {
        ...
        $sql = "INSERT INTO login_attempts (email, attempts, last_attempt) VALUES (?, 1, NOW())";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("s", $email);
        $stmt->execute();
    }
...
function resetFailedLoginAttempts($conn, $email)
{
    $sql = "UPDATE login_attempts SET attempts = 0, is_locked = 0 WHERE email = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $email);
    $stmt->execute();
}
...
{
    $sql = "UPDATE login_attempts SET is_locked = 1, lock_time = NOW() WHERE email = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $email);
    $stmt->execute();
}
...
{
    $permissions = getAdminPermissions($conn, $admin_id);
    if (isset($permissions[$permission])) {
        return $permissions[$permission] == 1;
    }
    $sql = "SELECT is_super_admin FROM admin_users WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $admin_id);
    $stmt->execute();
    $result = $stmt->get_result();
...
function updateOrderPaymentStatus($conn, $order_id, $status)
{
...
function getActivePopupAd($conn)
{
...
function getProductReviews($conn, $product_id, $limit = 0)
{
    $reviews = [];
...
function hasUserReviewedProduct($conn, $user_id, $product_id)
{
...
function getUserProductReview($conn, $user_id, $product_id)
{
    $sql = "SELECT * FROM product_reviews WHERE user_id = ? AND product_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ii", $user_id, $product_id);
...
function saveProductReview($conn, $user_id, $product_id, $rating, $comment)
{
    $existing_review = getUserProductReview($conn, $user_id, $product_id);
    if ($existing_review) {
        $sql = "UPDATE product_reviews SET rating = ?, comment = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
...
function getTopRatedProducts($conn, $limit = 8)
{
    $products = [];
    ...
    if (count($products) < $limit) {
      ...
      $id_list = empty($existing_ids) ? "" : implode(',', $existing_ids);

      $additional_sql = "SELECT * FROM products 
                        " . (!empty($id_list) ? "WHERE id NOT IN ($id_list)" : "") . "
                        ORDER BY created_at DESC 
                        LIMIT ?";
      $additional_stmt = $conn->prepare($additional_sql);
      $additional_stmt->bind_param("i", $needed);
